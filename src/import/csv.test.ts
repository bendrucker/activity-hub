...
const CSV = `Activity ID,Activity Date,Activity Name,Activity Type,Elapsed Time,Distance,Filename,Elapsed Time,Moving Time,Media
19324502491,"Jul 15, 2026, 1:11:54 PM",Headlands,Ride,6770,21.5,activities/20443348595.fit.gz,6770.0,4484.0,media/e21b4ad8.jpg||
593377978,"Jul 3, 2013, 1:10:23 PM",,Virtual Ride,553,2.1,activities/648066488.gpx,,474.0,
593378009,"Mar 20, 2013, 12:42:06 AM",Commute,E-Bike Ride,4095,30.0,,4095.0,3870.0,media/a.jpg|media/b.jpg|
`;
...
describe("parseActivitiesCsv", () => {
  ...
  it("reads a blank title as no name", () => {
    const rows = parseActivitiesCsv(CSV);
    expect(rows.map((row) => row.name)).toEqual(["Headlands", null, "Commute"]);
  });
